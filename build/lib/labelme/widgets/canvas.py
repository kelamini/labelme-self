...
class Canvas(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.setFont(QtGui.QFont('Decorative', int(15 * pixel)))
        for shape in self.shapes:
            if (shape.selected or not self._hideBackround) and self.isVisible(
                ...
                shape.paint(p)
            if len(shape.points) == 2 and shape.group_id != None:
                ctx = (shape.points[0].x() + shape.points[1].x())/2
                cty = (shape.points[0].y() + shape.points[1].y())/2                
                p.drawText(int(ctx),int(cty), str(shape.group_id))
